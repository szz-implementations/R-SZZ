...
public class Utils {
	...
	/**
	 * Returns the most recent insertion bug commit (vertex) in json format
	 * @param insertionBugCommit
	 * @param repositoryPath
	 * @return
	 */
	public static String mostRecentVertex(Set<Vertex> insertionBugCommit, String repositoryPath) {
		String ret = "[";

		Date auxDate = null;
		Vertex auxVertex = null;
		for (Vertex currentVertex : insertionBugCommit) {
			Date currentVertexDate = GitUtils.getInstance().getDateTime(currentVertex.getHash(), repositoryPath).getTime();

			if(auxDate == null){
				auxDate = currentVertexDate;
				auxVertex = currentVertex;
			}else if(currentVertexDate.after(auxDate)){
				auxDate = currentVertexDate;
				auxVertex = currentVertex;
			}
		}

		if (auxDate != null && auxVertex != null) {
			SimpleDateFormat s = new SimpleDateFormat("dd/MM/yyyy HH:mm:ss");
			String date = s.format(auxDate);
